...
enum CL_PROTOCOL
{
	...
	s_nCL_LoginREQ_Stream,			// 请求登录 替换 s_nCL_LOGIN_REQ 改成流模式 解决包太大的问题
	s_nCL_PacketEnd,
};
...
struct CL_LoginREQ_Stream
{
	...
	enum
	{
		enum_Key_PassportType = 1,	// 帐号类型 DWORD
		enum_Key_PassportName,		// 帐号名 string
		enum_Key_Password,			// 密码	string
		enum_Key_MainVersion,		// 主版本号 DWORD
		enum_Key_SubVersion,		// 子版本号 DWORD
		enum_Key_Version,			// 版本号 FLOAT
		enum_Key_MacAddr,			// 机器MAC地址 string
		enum_Key_RandomKey,			// 随机密码 INT64
		enum_Key_Accelerate_Platform,// 加速器开关 st_Accelerate_Platform(BYTE,BYTE,SHORT,DWORD)
		enum_Key_Langueage,			 // 语言 int
		enum_Key_NetType,			 // 网吧类型 int
	};
	/*
	st_LoginInfo loginInfo;
	DWORD m_dwMainVersion;
