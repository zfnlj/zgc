...
public:
	...
	template<typename R1, typename R2, typename T1> bool ret2TableCall(R1& ret1, R2& ret2, const char* name, const T1& arg1)
	{
		lua_State* L = LuaWraper.m_l;
		KLuaStackRecover sr(L);
		int errfunc = LMREG::lua_geterrfunc(L);
		...
		lua_pushvalue(L, -2);
		LMREG::push<T1>(L, arg1);
		lua_pcall(L, 2, 2, errfunc);
		ret1 = LMREG::read<R1>(L, -1);
		ret2 = LMREG::read<R2>(L, -2);
		return true;
	}

	template<typename R1, typename R2, typename R3, typename T1> bool ret3TableCall(R1& ret1, R2& ret2, R3& ret3, const char* name, const T1& arg1)
	{
		lua_State* L = LuaWraper.m_l;
		KLuaStackRecover sr(L);
		int errfunc = LMREG::lua_geterrfunc(L);
		...
		lua_pushvalue(L, -2);
		LMREG::push<T1>(L, arg1);
		lua_pcall(L, 2, 3, errfunc);
		ret1 = LMREG::read<R1>(L, -1);
		ret2 = LMREG::read<R2>(L, -2);
		ret3 = LMREG::read<R3>(L, -3);
		return true;
	}
...
public:
	bool foreachpair(KLuaTableVisitor& visitor)
	{
		lua_State* L = LuaWraper.m_l;
		KLuaStackRecover sr(L);
		this->loadTableInstance(L);
		...
		while (lua_next(L, t) != 0) {
			...
			int key = val - 1;
			{
				KLuaStackRecover __sr(L);
				if(!visitor.onpair(L, key, val))
					return false;
			}
...
public:
	...
	int getTableField_i32(int key, int defVal)
	{
		...
		lua_pushinteger(L, key);
		lua_gettable(L, -2);
		int val = lua_isnumber(L, -1) ? (int)lua_tointeger(L, -1) : defVal;
		lua_pop(L, 2);
		return val;
	}
...
public:
	...
	KLuaTableWraper(int index=LUA_GLOBALSINDEX)
	{
		lua_State* L = LuaWraper.m_l;
	...
	KLuaTableWraper(const char* tblname)
	{
		lua_State* L = LuaWraper.m_l; ASSERT(L);
	...
	template <typename T> KLuaTableWraper(KLuaTableHolder<T>& holder)
	{
		lua_State* L = LuaWraper.m_l; ASSERT(L);
	...
	KLuaTableWraper(const KLuaTableWraper& o)
	{
		lua_State* L = LuaWraper.m_l;
		this->releaseTableInstance();
