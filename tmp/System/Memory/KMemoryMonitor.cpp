...
extern System::Collections::KString<512> traceback_str(const CallStack& callstack, int n);
...
void KMemoryMonitor::dumpResult()
{
	if(!m_allocMap.empty())
	{
		...
		while(it != ite)
		{
			...
			System::Collections::KString<512> bt = traceback_str(u.callStack, sizeof(u.callStack)/sizeof(uint_r));
			Log(LOG_WARN, "memory: malloc, size:%u ptr:%08p, %s", size, p, bt.c_str());
		}
		m_allocMap.clear();
	}
	if(!m_freeMap.empty())
	{
		...
		while(it != ite)
		{
			...
			mu& u = it->second;
			System::Collections::KString<512> bt = traceback_str(u.callStack, sizeof(u.callStack)/sizeof(uint_r));
			Log(LOG_WARN, "memory: free, ptr:%08p, %s", p, bt.c_str());
			it++;
		}
		m_freeMap.clear();
	}
