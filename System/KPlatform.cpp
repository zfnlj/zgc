...
EnumEncoding g_encoding = encode_utf8;
...
DWORD GetModuleFileName_k(HMODULE hModule, char* lpFilename, DWORD nSize)
{
    ...
    /*
	char path[512];
	DWORD n = GetModuleFileNameA(hModule, path, sizeof(path));
	if(!n) return 0;
...
DWORD GetCurrentDirectory_k(DWORD nBufferLength, char* lpBuffer)
{
    ...
    /*
	char path[512];
	DWORD n = GetCurrentDirectoryA(sizeof(path), path);
	if(!n) return 0;
...
{
	HKEY hUser; 
	LONG ret = RegOpenKeyExA(HKEY_LOCAL_MACHINE, "HARDWARE\\DESCRIPTION\\System\\CentralProcessor\\0", 0, KEY_READ, &hUser);
	...
	ret = RegQueryValueExA(hUser, "~MHz", NULL, &valType, (BYTE*)&buf, &len);
	if(ret != ERROR_SUCCESS) return 1000;
...
BOOL assureOneProgramInstance()
{
	...
	if(!CreateMutexA(NULL, FALSE, fileName))
	{
...
{
	char buf[512];
	GetModuleFileName_k(NULL, buf, sizeof(buf));
