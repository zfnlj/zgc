...
void LuaInclude(char* cafile, int udir)
{
	...
	if (ar.source[0] == '@')
	{
		...
		{
#ifdef WIN32
			GetCurrentDirectoryA(MAX_PATH, caPath); 
#else
...
int KLuaWrap::doDirScripts(const char* dir)
{
	...
	for(int i=0; i<fileList.size(); i++)
	{
		...
		{
			nRet = scriptResult;
			Log(LOG_FATAL, "error: KLuaWrap::doDirScripts, open file %s", fullPath);
			ASSERT(FALSE);
		}
	}
...
{
	return luaL_loadstream_k(m_l, &si, NULL) == lwp_success;
}

BOOL KLuaWrap::loadStream(StreamInterface& si,const char* fn)
{
	return luaL_loadstream_k(m_l, &si, fn) == lwp_success;
}
...
BOOL KLuaWrap::doStream(StreamInterface& si)
{
	...
	if(!this->loadStream(si)) return FALSE;
...
BOOL KLuaWrap::doStream(StreamInterface& si,const char* fn)
{
	KLuaStackRecover sr(m_l);
	int errfunc = LMREG::lua_geterrfunc(m_l);
	if(!this->loadStream(si,fn)) return FALSE;
	if(lua_pcall(m_l, 0, LUA_MULTRET, errfunc))
		return FALSE;
	return TRUE;
}
...
namespace LMREG
{
	...
	void print_error(lua_State *L, const char* fmt, ...)
	{
		...
		g_luaLogger.WriteLog(LOG_WARN, "%s", text);
		...
		#if defined(WIN32) || defined(WINDOWS)
		if (LuaWraper.m_funcErrReport) LuaWraper.m_funcErrReport(L);
			...
			case LUA_TNUMBER:
				{
					...
					//if(fabs(num-n) < 0.000001)
					//{
					//	if(n < ((INT64)1<<32))
					//	{
					//		ret[key.c_str()] = (unsigned int)n;
					//	}
					//	else
					//	{
					//		ret[key.c_str()] = (UINT64)n;
					//	}
					//}
					//else
					//{
			...
			case LUA_TLIGHTUSERDATA:
				{
					...
					ret.setobject(key.c_str(), p, "lightuserdata");
				} break;
			case LUA_TFUNCTION:
				{
...
// 				if(lua_isnil(L, -1))
// 				{
// 					lua_pushfstring(L, "can't find '%s' class variable. (forgot registering class variable ?)", lua_tostring(L, 2));	// userdata key metatable retvalue string
// 					lua_error(L);										// userdata key metatable retvalue
// 				}
			}
		} 
