...
namespace LMREG
{
	...
	public:
		template<size_t ln> JG_C::KDString<ln> toString() const
		{
			...
			case vt_int32:
				{
					char buf[64];
					int n = sprintf(buf, "%d", this->n);
					return s_t(buf, n);
				} break;
			case vt_uint32:
				{
					char buf[64];
					int n = sprintf(buf, "%u", this->un);
					return s_t(buf, n);
				} break;
			case vt_int64:
				{
					char buf[64];
					int n = sprintf(buf, "%lld", this->ll);
					return s_t(buf, n);
				} break;
			case vt_uint64:
				{
					char buf[64];
					int n = sprintf(buf, "%llu", this->ull);
					return s_t(buf, n);
				} break;
			case vt_float:
				{
					char buf[64];
					int n = sprintf(buf, "%f", this->f);
					return s_t(buf, n);
				} break;
			case vt_double:
				{
					char buf[64];
					int n = sprintf(buf, "%f", this->d);
					return s_t(buf, n);
				} break;
			case vt_string:
				{
			...
			case vt_object:
				{
					char buf[256];
					int n = sprintf(buf, "%s 0x%08x", this->obj.first, this->obj.second);
					return s_t(buf, n);
				} break;
			}
			ASSERT(FALSE);
	...
	private:
		char m_baseType[sizeof(_CppTable_base)];
	...
	private:
		base_type& _baseType() { return *(base_type*)&m_baseType; }
		const base_type& _baseType() const { return *(base_type*)&m_baseType; }
