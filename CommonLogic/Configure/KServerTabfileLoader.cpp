...
{
	Log(LOG_ERROR, "tabfile: begin read %s by tabfile2", filename);
...
{
	Log(LOG_ERROR, "tabfile: begin read %s by original tabfile", filename);
...
StreamInterface* KServerTabfileLoader::GetFileStream(const char* filename, const char* grp)
{
	KServerTabFileReader* pFileReader = KServerTabFileReader::Alloc();
	ASSERT(pFileReader); pFileReader->Close();
	KInputFileStream& fi = pFileReader->m_istream;
	if(KFileUtil::IsFileExists(filename))
	{
		if(!fi.Open(filename))
		{
			pFileReader->Release();
...
void KServerTabfileLoader::CloseFileStream(StreamInterface* stream)
{
	if(!stream) return;
	size_t offt = offsetof(KServerTabFileReader,m_istream);
	KInputFileStream* p = (KInputFileStream*)stream;
	KServerTabFileReader* r = (KServerTabFileReader*)((char*)p-offt);
	r->Release();
}
