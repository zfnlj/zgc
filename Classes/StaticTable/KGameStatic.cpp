...
KHelpStringStatic* KGameStaticMgr::GetLessonTip(int id)
{
	KHelpStringMap::iterator it  = m_lessonTipMap.find(id);
	if(it==m_lessonTipMap.end()) return NULL;
	return it->second;
}
...
int KGameStaticMgr::GetRankCardNum(int rank)
{
	int count =0;
	for(CardMap::iterator it = m_cardMap.begin();it!=m_cardMap.end();++it){
		KCardStatic* pCard = it->second;
		if(pCard->GetRank()==rank){
